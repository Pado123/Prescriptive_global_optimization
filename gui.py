    ...
    if n_clicks >= 1:
        for i in range(4):
            ...
            new_dropdown = dbc.Row(
                        [dbc.Col(html.Div(name.split(' ')[2] + ' column'), width=1),
...
def print_expected_time(n_clicks):
    if n_clicks > 0:
        ...
        # min = (seconds%3600)//60
        # print(df.shape)
        return f'The model is being trained'
...
def print_expected_time_gen(n_clicks):
    if n_clicks > 0:
        ...
        # min = (seconds % 3600) // 60
        # print(df.shape)
        return 'The predictions are being generated'
...
def generate_predictions(n_clicks):
    if n_clicks > 0:
        ...
        return 'The predictions are generated'
